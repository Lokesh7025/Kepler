...
Kepler must execute model-selected actions inside an enforceable sandbox. The same canonical tools should work against:
...
- No published ports
- Cgroups v2 limits
- Egress only through the Kepler proxy
...
For a hosted session, the authoritative loop runs on the worker. Clients attach through the normal Kepler protocol. The local daemon coordinates placement and transfer but does not run a second loop.
...
The workspace remains remote for the session. Results return as a verified snapshot, patch, branch, or pull request. Kepler must not continuously copy individual file writes between local and hosted filesystems.
...
Exit: a Kepler session runs in a disposable local container and cleanup is verified.
...
Exit: a local Kepler client safely runs a session on another machine and verifies cleanup.
