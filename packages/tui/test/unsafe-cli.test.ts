...
import { KeplerDaemon, DaemonClient, type SessionSnapshot } from "@kepler/daemon";
import { type ModelPort, ToolRegistry } from "@kepler/kernel";
import type { ModelStreamEvent } from "@kepler/protocol";

const entry = fileURLToPath(new URL("../dist/main.js", import.meta.url));

async function temporaryDirectory(context: TestContext): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "kepler-unsafe-cli-"));
...
test("--unsafe starts a separate unenforced daemon and records the warning state", async (context) => {
  ...
  const stateDirectory = join(home, ".kepler", "unsafe");
  const socketPath = join(stateDirectory, "kepler.sock");
